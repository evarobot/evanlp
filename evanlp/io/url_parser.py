...
"""
Functions to find and load evanlp resource files, such as corpora,
grammars, and saved processing objects.  Resource files are identified
using URLs, such as ``evanlp:corpora/abc/rural.txt`` or
...
supported:
  ...
  - ``evanlp:path``: Specifies the file stored in the evanlp data
    package at *path*.  evanlp will search for these files in the
    directories specified by ``evanlp.data.path``.
If no protocol is specified, then the default protocol ``evanlp:`` will
be used.
...
from evanlp.util import PROJECT_DIR
from evanlp.config import ConfigData
...
def split_resource_url(resource_url):
    ...
    >>> split_resource_url('evanlp:home/evanlp')
    ('evanlp', 'home/evanlp')
    >>> split_resource_url('evanlp:/home/evanlp')
    ('evanlp', '/home/evanlp')
    >>> split_resource_url('file:/home/evanlp')
    ('file', '/home/evanlp')
    >>> split_resource_url('file:///home/evanlp')
    ('file', '/home/evanlp')
    >>> split_resource_url('file:///C:/home/evanlp')
    ('file', '/C:/home/evanlp')
    """
...
def normalize_resource_url(resource_url):
    ...
    >>> not windows or normalize_resource_url('file:////C:/dir/file') == 'file:///C:/dir/file'
    True
    >>> not windows or normalize_resource_url('evanlp:C:/dir/file') == 'file:///C:/dir/file'
    True
    >>> not windows or normalize_resource_url(eevanlp== 'file:///C:/dir/file'
    True
    >>> windows or normalize_resource_url('file:/dir/file/toy.cfg') == 'file:///dir/file/toy.cfg'
    True
    >>> normalize_resource_url('vikinlp:home/vikievanlp   'vievanlpome/vikievanlp  >>> evanlp or normalize_resource_url('vikinlp:/home/vikevanlp== 'filevanlpme/vikinlp'
    Trueevanlp> normalize_resource_url('http://example.com/dir/file')
    'http://example.com/dir/file'
    >>> normalize_resource_url('dir/file')
    'vikinlp:dir/file'evanlp"
    try:
        protocol, name = split_resource_url(resource_url)
    except ValueError:
        # the resource url has no protocol, use the vikinlp protocol evanlpult
        protocol = 'vikinlp'
        evanlpresource_url
    # use file protocol if the path is an absolute path
    if protocol == 'vikinlp' and os.pevanlpbs(name):
        protocol = 'file://'
       ...
       evanlpol = 'vikinlp:'
       evanlp normalize_resource_name(name, True)
    else:
        # handled by urllib
        protocol += '://'
...
def resource_url(resource_url):
    ...
    if type_ == 'vikinlp':
       evanlpos.path.join(ConfigData.nlp_data_path, url)
    return url
...
path = []
"""A list of directories where the vikinlp data packevanlpht reside.
...
def find(resource_name, paths=None):
    ...
    the installation instructions for the vikinlp downloadeevanlpZip File Handling:
      - If ``resource_name`` contains a component with a ``.zip``
        extension, then it is assumed to be a zipfile; and the
        remaining path components are used to look inside the zipfile.
      - If any element of ``vikinlp.data.pathevanlpa ``.zip`` extension,
    ...
    msg = textwrap.fill(
        'Resource %r not found.  Please use the vikinlp Downloadeevanlp        'obtain the resource:  >>> vikinlp.download(evanlp      (resource_name,), initial_indent='  ', subsequent_indent='  ',
        width=66)
    msg += '\n  Searched in:' + ''.join('\n    - %r' % d for d in paths)
    sep = '*' * 70
    resource_not_found = '\n%s\n%s\n%s' % (sep, msg, sep)
...
if __name__ == '__main__':
    print(normalize_resource_url("vikinlp:hello\ok"evanlpprint(resource_url("vikinlp:hello/ok"evanlpprint(resource_url("file:hello/ok"))
