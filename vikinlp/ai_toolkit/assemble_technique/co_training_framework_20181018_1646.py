...
class CoTrainingClassifier(object):
	...
	def fit(self, X1, X2, y, **kwargs):
		...
		while it != self.k_ and U:
			...
			lst_x2_text = [X2_text[i] for i, x in enumerate(X1_text) if i in L]
			# lst_y_continuous = [Y_continuous[i] for i, x in enumerate(X1_text) if i in L]

			self.fit_dl(self.clf1_, X1[L], Y_dl[L], X1_text, y[L], kwargs)
			self.fit_dl(self.clf2_, X2[L], Y_dl[L], X1_text, y[L], kwargs)
		...
		# 深度学习模型
		self.fit_dl(self, self.clf1_, X1[L], y[L], X1_text[L], Y1_continuous[L], kwargs)
		...
		self.fit_dl(self, self.clf2_, X2[L], y[L], X2_text[L], Y2_continuous[L], kwargs)
		# if isinstance(self.clf2_, DeepLearningArchitecture):
